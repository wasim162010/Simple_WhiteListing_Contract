...
contract WLDataStorage {
   ...
   function getWhiteListAddress() public view returns(address[] memory) {
    ...
    function setDevices(address _device,address _mainAccnt) public { 
       /*
         require(deviceCountPerAddress[msg.sender] <=64);
       
         Devices[devicesIndexCount++][msg.sender] = _device; 
         DeviceToOwnerMapping[_device]=msg.sender;
         devicesIndexCount++;
         deviceCountPerAddress[msg.sender]++;
         */
    ...
    function IsWhiteList(address addr) public view returns(bool) {
        ...
        //if(DeviceToOwnerMapping[addr] == 0x0)
          //    isWL = false;
        //else 
          //  isWL =true;
